...
public final class OAuth2ClientCredentialsTokenProvider
    implements MarketBearerTokenProvider
{
    ...
    public synchronized String getBearerToken() {
        ...
        if (
            !root.has("access_token") || root.get("access_token").isJsonNull()
        ) {
            throw new IllegalStateException(
        ...
        long expiresIn =
            root.has("expires_in") && !root.get("expires_in").isJsonNull()
    ...
    private TokenResponse requestToken(boolean includeClientCredentialsInBody) {
        ...
        try {
            ...
            throw new IllegalStateException(
                "Interrupted while fetching market API bearer token.",
                exception
            );
        }
    }
