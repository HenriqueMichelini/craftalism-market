...
public final class MarketCommand implements CommandExecutor {
    ...
    public MarketCommand(Plugin plugin, MarketBrowseSnapshotService snapshotService, MarketGuiService guiService) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        snapshotService.loadForInitialOpen().whenComplete((result, error) ->
                plugin.getServer().getScheduler().runTask(plugin, () -> handleResult(player, result, error))
        );
        return true;
    }

    private void handleResult(Player player, MarketBrowseSnapshotLoadResult result, Throwable error) {
        if (!player.isOnline()) {
            return;
        }
        ...
        String messagePath = result.fromCache() ? "messages.opened-read-only" : "messages.opened-live";
        player.sendMessage(ChatColor.translateAlternateColorCodes(
                '&',
                plugin.getConfig().getString(messagePath, "&7Market data loaded.")
        ));
    }
}
