...
public final class MarketApiConfigurationResolver {
    ...
    public MarketApiConfiguration resolve() {
        return new MarketApiConfiguration(
                normalizeBaseUrl(firstNonBlank(
                        environment.get("CRAFTALISM_API_URL"),
                        config.getString("market-api.base-url", DEFAULT_BASE_URL)
                )),
                normalizePath(config.getString("market-api.snapshot-path", "/market/snapshot"), "/market/snapshot"),
                normalizePath(config.getString("market-api.quote-path", "/market/quote"), "/market/quote"),
                normalizePath(config.getString("market-api.execute-path", "/market/execute"), "/market/execute"),
                blankToEmpty(config.getString("market-api.auth-token", "")),
                normalizeOptionalBaseUrl(firstNonBlank(
                        environment.get("AUTH_ISSUER_URI"),
                        config.getString("market-api.auth.issuer-url", "")
                )),
                normalizeTokenPath(firstNonBlank(
                        environment.get("AUTH_TOKEN_PATH"),
                        config.getString("market-api.auth.token-path", DEFAULT_TOKEN_PATH)
                )),
                blankToEmpty(firstNonBlank(
                ...
                Duration.ofMillis(normalizeTimeout(config.getLong("market-api.connect-timeout-ms", 3000L), 3000L, "market-api.connect-timeout-ms")),
                Duration.ofMillis(normalizeTimeout(config.getLong("market-api.request-timeout-ms", 5000L), 5000L, "market-api.request-timeout-ms"))
        );
    }
    ...
    private long normalizeTimeout(long value, long fallback, String key) {
        ...
        logger.warning("Invalid timeout for " + key + " (" + value + "), falling back to " + fallback + "ms.");
        return fallback;
    }
    ...
    private String normalizeBaseUrl(String value) {
        ...
        logger.warning("Invalid market API base URL '" + value + "', falling back to " + DEFAULT_BASE_URL);
