...
public final class MarketPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        MarketApiConfiguration apiConfiguration =
            new MarketApiConfigurationResolver(
                getConfig(),
        ...
        MarketBrowseSnapshotProvider snapshotProvider =
            new HttpMarketBrowseSnapshotProvider(
                apiTransport,
                URI.create(
                    apiConfiguration.baseUrl() + apiConfiguration.snapshotPath()
                ),
        ...
        MarketQuoteClient quoteClient = new HttpMarketQuoteClient(
            apiTransport,
            URI.create(
                apiConfiguration.baseUrl() + apiConfiguration.quotePath()
            ),
            apiConfiguration.requestTimeout(),
            bearerTokenProvider
        );
        MarketExecuteClient executeClient = new HttpMarketExecuteClient(
            apiTransport,
            URI.create(
                apiConfiguration.baseUrl() + apiConfiguration.executePath()
            ),
            apiConfiguration.requestTimeout(),
            bearerTokenProvider
        );
        ...
        MarketBrowseSnapshotService snapshotService =
            new MarketBrowseSnapshotService(snapshotProvider, asyncExecutor);
        ...
        if (getCommand("market") == null) {
            throw new IllegalStateException(
                "The /market command must be declared in plugin.yml."
            );
        }

        getCommand("market").setExecutor(
            new MarketCommand(this, snapshotService, guiService)
        );
        getServer()
            .getPluginManager()
